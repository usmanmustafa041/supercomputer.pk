import type { Product } from "@supercomputers/shared";
  ...
  switch (p.kind) {
    case "gpu":
      return [
        ["Silicon", [
          ["Architecture", p.arch],
          ["Form factor", p.formFactor === "pcie" ? `PCIe ${p.pcieGen}.0 x${p.pcieWidth}` : `${upper(p.formFactor)} module`],
    ...
    case "cpu":
      return [
        ...
        ["Memory controller", [
          ["Generation", upper(p.memGen)],
          ["Channels", `${p.memChannels} per socket`],
          ["Accepted", upper(p.memKinds?.join(", "))],
    ...
    case "storage":
      return [
        ...
        ["Performance", [
          ...
          ["Random read", `${p.readIops.toLocaleString("en-GB")} IOPS`],
          ["Random write", `${p.writeIops.toLocaleString("en-GB")} IOPS`],
        ]],
        ["Endurance & power", [
    ...
    case "chassis":
      return [
        ["Form", [
          ...
          ["Board support", upper(p.moboForms?.join(", "))],
          ["Supply support", upper(p.psuForms?.join(", "))],
        ]],
        ["Clearance", [
        ...
        ["Storage & airflow", [
          ...
          ["Backplane", p.backplane === "none" ? "None, internal mounting only" : upper(p.backplane)],
    ...
    case "system":
      return [
        ...
        ["Memory & storage", [
          ["Memory", `${p.memGb >= 1024 ? `${p.memGb / 1024} TB` : `${p.memGb} GB`} ${upper(p.memGen)}`],
        ...
        ["Physical & power", [
          ...
          ["Heat rejected", `${Math.round(p.peakPowerW * 3.412).toLocaleString("en-GB")} BTU/hr`],
        ]],
        ["Delivered with", [
