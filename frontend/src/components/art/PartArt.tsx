...
import type { Kind, Product } from "@supercomputers/shared";

/**
 * Each category's own colour.
 *
 * Fifteen unrelated hues would look like a toy shop. These are six families,
 * shared out by what the part actually does, so a catalogue page reads as a
 * system rather than a paint chart: anything that computes runs warm, anything
 * that moves data runs cyan, boards are board-green, power is amber, metalwork
 * is steel, and memory and storage take the violet end.
 *
 * The colour is never decoration on its own. It lands on the working parts,
 * connectors, fins, ports, the power rail, so it reads as an annotation.
 */
const KEYS: Record<Kind, [key: string, second: string]> = {
  gpu: ["#FF5A1F", "#FFB020"],
  cpu: ["#FF7A45", "#FFD166"],
  system: ["#FF4D2E", "#FFA03C"],

  memory: ["#9B7CFF", "#54D6F0"],
  storage: ["#7C6BFF", "#2ED3A7"],

  motherboard: ["#3FBF63", "#9B7CFF"],

  psu: ["#FFB020", "#FF7A45"],
  pdu: ["#FFA22B", "#FF5A1F"],
  ups: ["#E0C93A", "#3FBF63"],

  chassis: ["#5B8DEF", "#8FA6C4"],
  rack: ["#6E8CA8", "#5B8DEF"],
  cooler: ["#4FC9F0", "#B06AE0"],

  nic: ["#29D3EE", "#3FBF63"],
  switch: ["#22B8E8", "#FFB020"],
  optic: ["#31D6C8", "#FFD166"],
};

/** What goes in the drawing's title block. Short enough to stay one line. */
...
/**
 * The category's colours, resolved per drawing.
 *
 * Written as CSS variables rather than literals so the whole drawing takes its
 * palette from one declaration on the <svg>, and so a drawing dropped anywhere
 * else on the page inherits sensible values instead of breaking.
 */
const KEY = "var(--art-key)";
const KEY2 = "var(--art-key-2)";

/** Catalogue records can contain incomplete legacy specs; artwork must not
 * take down the whole page because one label is missing. */
...
function GpuArt({ p, s }: { p: Extract<Product, { kind: "gpu" }>; s: number }) {
  if (p.formFactor !== "pcie") {
    ...
    return (
      <g>
        ...
        <rect x="82" y="58" width="36" height="34" fill="var(--art-void)" stroke={KEY} strokeWidth="1.2" />
        {[0, 1, 2, 3].map((i) => (
          <rect key={i} x={62 + i * 22} y="108" width="14" height="6" fill={INK} opacity="0.5" />
        ))}
        {[0, 1, 2, 3].map((i) => (
          <rect key={i} x={58} y={38 + i * 20} width="6" height="12" fill={KEY2} opacity="0.35" />
        ))}
        <text x="100" y="136" fill={INK} fontSize="7" fontFamily="monospace" textAnchor="middle" letterSpacing="1.5">
          {upper(p.formFactor)} MODULE
        </text>
      </g>
    );
  }
  ...
  return (
    <g>
      ...
      <rect x={x0} y={y0 + h - 11} width={w} height="1.6" fill={p.eccVram ? KEY2 : KEY} opacity="0.85" />
      ...
      {p.connectors.length > 0 && (
        <g>
          {p.connectors.slice(0, 3).map((c, i) => (
            <rect key={i} x={x0 + w - 34 + i * 11} y={y0 - 4} width="9" height="5" fill="var(--art-void)" stroke={c.startsWith("12v") ? KEY : INK_HI} strokeWidth="0.8" />
          ))}
        </g>
      )}
    </g>
...
function CpuArt({ p, s }: { p: Extract<Product, { kind: "cpu" }>; s: number }) {
  ...
  return (
    <g>
      ...
      <path d={`M ${x0 + 2} ${y0 + 10} L ${x0 + 2} ${y0 + 2} L ${x0 + 10} ${y0 + 2} Z`} fill={KEY} opacity="0.8" />
      {/* laser etch lines */}
      {[0, 1, 2].map((i) => (
      ...
      {pick(s, 3, 0, 1) === 1 && <circle cx={x0 + w - 8} cy={y0 + h - 8} r="1.6" fill={KEY2} opacity="0.6" />}
    </g>
  );
}
      ...
      {Array.from({ length: n }).map((_, i) => {
        ...
        return (
          <g key={i}>
            ...
            {p.registered && <rect x="96" y={y + 1.5} width="9" height="5" fill="var(--art-fill-4)" stroke={KEY} strokeWidth="0.7" />}
            {/* keying notch */}
...
function DriveArt({ p, s }: { p: Extract<Product, { kind: "storage" }>; s: number }) {
    ...
    return (
      <g>
        ...
        {p.powerLossProtection &&
          [0, 1, 2].map((i) => <rect key={i} x={100 - len / 2 + 80 + i * 9} y="72" width="6" height="8" fill="var(--art-fill-4)" stroke={KEY} strokeWidth="0.6" />)}
  ...
  return (
    <g>
      ...
      {/* connector edge */}
      <rect x={100 - w / 2} y={76 + h / 2 - 14} width="4" height="24" fill={p.bus === "sata" ? "var(--art-fill-4)" : KEY} opacity="0.7" />
      <text x="100" y={76 + h / 2 + 16} fill={INK} fontSize="7" fontFamily="monospace" textAnchor="middle" letterSpacing="1.4">
          {upper(p.bus)} · {p.physical ?? "—"}
      </text>
      {pick(s, 5, 0, 2) === 0 && <circle cx={100 + w / 2 - 8} cy={76 - h / 2 + 8} r="1.5" fill={KEY2} opacity="0.5" />}
    </g>
  );
}
    ...
    <g>
      ...
      {p.pcieSlots.slice(0, 7).map((sl, i) => (
        <g key={i}>
          ...
          {sl.gen >= 5 && <rect x={x0 + 10} y={70 + i * 8.5} width="3" height="4.5" fill={KEY} opacity="0.7" />}
        </g>
      ))}
      {/* M.2 */}
      ...
      {p.ipmi && <circle cx={x0 + 4.5} cy="64" r="2.2" fill={KEY2} opacity="0.65" />}
      {/* mounting holes */}
    ...
    <g>
      ...
      {redundant ? (
        <>
          {[0, 1].map((i) => (
            <g key={i}>
              ...
              <circle cx="140" cy={57 + i * 38} r="2" fill={i === 0 ? "var(--art-led)" : KEY} opacity="0.7" />
            </g>
          ))}
        ...
        <>
          ...
          <text x="100" y="128" fill={INK} fontSize="7" fontFamily="monospace" textAnchor="middle" letterSpacing="1.4">
            {upper(p.form)} · {upper(p.efficiency).replace("80+ ", "")}
          </text>
        </>
      )}
      <text x="100" y="26" fill={KEY} fontSize="13" fontFamily="monospace" textAnchor="middle" fontWeight="600" letterSpacing="0.5">
        {p.wattage}W
      </text>
    </g>
  );
  ...
  if (rack) {
    ...
    return (
      <g>
        ...
        {p.forcedAirflow &&
          [0, 1, 2].map((i) => (
            <path key={i} d={`M ${150} ${76 - 8 + i * 8} l 12 0 m -3 -3 l 3 3 l -3 3`} fill="none" stroke={KEY2} strokeWidth="0.9" opacity="0.55" />
          ))}
    ...
    <g>
      {open ? (
        ...
        <>
          ...
          <rect x="114" y="60" width="22" height="9" fill="var(--art-fill-2)" stroke={KEY} strokeWidth="0.6" opacity="0.7" />
      ...
      {isSwitch ? (
        <>
          ...
          <text x="100" y="114" fill={INK} fontSize="7.5" fontFamily="monospace" textAnchor="middle" letterSpacing="1.5">
            {p.ports}x {p.portGbps}G {upper(p.portType)}
          </text>
        </>
      ) : (
        <>
          ...
          {p.gpuDirect && <circle cx="150" cy="60" r="2" fill={KEY} opacity="0.8" />}
          <text x="100" y="114" fill={INK} fontSize="7.5" fontFamily="monospace" textAnchor="middle" letterSpacing="1.5">
            {p.portGbps}G {upper(p.portType)} · x{p.pcieWidth}
          </text>
        </>
      )}
    </g>
  ...
  if (p.type === "aio") {
    return (
      <g>
        ...
        <circle cx="135" cy="105" r="7" fill="none" stroke={KEY} strokeWidth="1" opacity="0.8" />
...
function RackArt({ p }: { p: Extract<Product, { kind: "rack" }> }) {
  ...
  return (
    <g>
      ...
      <text x="100" y="147" fill={INK} fontSize="7.5" fontFamily="monospace" textAnchor="middle" letterSpacing="1.6">
    ...
    <g>
      ...
      {isUps && (
        <>
          ...
          <rect x="34" y="62" width="38" height="14" fill="var(--art-glass)" stroke={KEY2} strokeWidth="0.6" opacity="0.6" />
          {[0, 1, 2, 3].map((i) => (
    ...
    <g>
      {cable ? (
        <>
          ...
          <path d="M 58 74 C 84 46, 116 102, 142 74" fill="none" stroke={p.media.startsWith("dac") ? INK_HI : KEY2} strokeWidth={p.media.startsWith("dac") ? 5 : 2.4} opacity="0.75" strokeLinecap="round" />
          <text x="100" y="106" fill={INK} fontSize="7.5" fontFamily="monospace" textAnchor="middle" letterSpacing="1.5">
            {p.lengthM}M {upper(p.media)} · {p.gbps}G
          </text>
        </>
      ) : (
        <>
          ...
          <rect x="128" y="66" width="18" height="7" fill="var(--art-glass)" stroke={KEY2} strokeWidth="0.6" opacity="0.7" />
          <rect x="128" y="76" width="18" height="7" fill="var(--art-glass)" stroke={KEY2} strokeWidth="0.6" opacity="0.7" />
          <path d="M 74 74 l 46 0" stroke={INK} strokeWidth="0.8" strokeDasharray="3 2" />
          <text x="100" y="106" fill={INK} fontSize="7.5" fontFamily="monospace" textAnchor="middle" letterSpacing="1.5">
            {upper(p.portType)} {upper(p.media)} · {p.reachM}M
          </text>
        </>
      )}
    </g>
    ...
    return (
      <g>
        ...
        {Array.from({ length: Math.min(4, p.gpuCount) }).map((_, i) => (
          <rect key={i} x="66" y={72 + i * 13} width="68" height="9" fill="var(--art-fill-2)" stroke={KEY} strokeWidth="0.6" opacity="0.85" />
        ))}
...
export default function PartArt({ product: p, className = "", bare = false }: Props) {
  ...
  return (
    <svg
      ...
      // Declared once, here, and read by every shape below. One category, one
      // palette, and nothing downstream has to know which category it is.
      style={{ "--art-key": key, "--art-key-2": key2 } as React.CSSProperties}
    >
      <defs>
        ...
        {/* The colour behind the part. Strongest where the part sits, thinning
            towards the edges, so a grid has real variety without fifteen flat
            rectangles of poster paint fighting each other.
            The first version of this was so faint it may as well not have been
            there: on a near-black panel a 7% tint is invisible. */}
        <radialGradient id={`w-${p.id}`} cx="50%" cy="42%" r="72%">
          <stop offset="0%" stopColor={KEY} stopOpacity="0.30" />
          <stop offset="45%" stopColor={KEY} stopOpacity="0.16" />
          <stop offset="100%" stopColor={KEY2} stopOpacity="0.05" />
        </radialGradient>

        {/* Left-lit, like a part photographed on a bench. Flat fills are what
            made the earlier drawings read as clip art. */}
        <linearGradient id={`l-${p.id}`} x1="0" y1="0" x2="0.9" y2="1">
          <stop offset="0%" stopColor="#FFFFFF" stopOpacity="0.09" />
          <stop offset="45%" stopColor="#FFFFFF" stopOpacity="0" />
          <stop offset="100%" stopColor="#000000" stopOpacity="0.14" />
        </linearGradient>

        <linearGradient id={`k-${p.id}`} x1="0" y1="0" x2="1" y2="0">
          <stop offset="0%" stopColor={KEY} stopOpacity="0.95" />
          <stop offset="70%" stopColor={KEY2} stopOpacity="0.5" />
          <stop offset="100%" stopColor={KEY2} stopOpacity="0" />
        </linearGradient>
      </defs>

      <rect width="200" height="152" fill={`url(#w-${p.id})`} />
      ...
      {/* The lighting pass sits over the part, not under it, and is set not to
          catch pointer events so it cannot interfere with anything on top. */}
      <rect width="200" height="152" fill={`url(#l-${p.id})`} pointerEvents="none" />

      {/* The category's colour, stated plainly along the bottom edge. This is
          what makes a mixed grid scannable: power supplies are the amber row,
          network cards the cyan one, before a single label is read. */}
      <rect x="0" y="149" width="200" height="3" fill={`url(#k-${p.id})`} />

      {/* Title block, the corner of a real parts drawing. Also the one place
          the colour is at full strength, which is what stops the whole thing
          reading as a grey sketch someone tinted afterwards. */}
      <g pointerEvents="none">
        <rect x="0" y="132" width="34" height="17" fill={KEY} opacity="0.92" />
        <text
          x="17"
          y="143.5"
          fill="#0B0704"
