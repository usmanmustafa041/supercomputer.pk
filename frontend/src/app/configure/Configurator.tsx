...
import { SLOTS, slotsFor, type PresetView, type Slot } from "./slots";
import { CONDITION_LABEL, type Kind, type Product } from "@supercomputers/shared";
import { checkBuild } from "@supercomputers/shared";
import { TARGET_LABEL, type BuildLine, type Finding, type Target } from "@supercomputers/shared";
...
export default function Configurator({
  ...
  /** Said out loud when a preset loads short or not at all. */
  ...
  /**
   * Replaces the configuration with a pre-built one.
   *
   * A line that cannot be matched to anything we currently list is counted and
   * reported rather than quietly dropped. Silently loading six parts of an
   * eight part configuration is worse than loading none: it looks like it
   * worked, and the missing power supply only surfaces at the quote.
   */
  const loadPreset = useCallback(
    (slug: string) => {
      ...
      const wanted = preset.picks
        .map((k) => (k.variant ? `${k.family}:${encodeURIComponent(k.variant)}` : k.family))
        .join(",");

      fetch(`/api/catalog?families=${wanted}`)
        .then((r) => r.json())
        .then((d: { items: Product[] }) => {
          ...
          for (const pick of preset.picks) {
            const product = byFamily.get(pick.family);
            if (!product) {
              missing++;
              continue;
            }
            next.push({ product, qty: Math.max(1, Math.round(pick.qty) || 1) });
          }
          setTarget(preset.target);
          setLines(next);
          if (next.length === 0) {
            setPresetNote(`Nothing in ${preset.name} could be matched to what we currently list.`);
          } else if (missing > 0) {
            setPresetNote(
              `Loaded ${preset.name}, but ${missing} line${missing === 1 ? "" : "s"} could not be matched to anything we currently list. Check the parts list.`,
            );
          }
        })
        .catch(() => setPresetNote(`Could not load ${preset.name}. Check your connection and try again.`))
    ...
    <div className="shell py-4 md:py-9 pb-28 lg:pb-9 overflow-x-clip">
      <header className="mb-3 lg:mb-5">
        ...
        <div className="flex flex-wrap items-end justify-between gap-3">
          <h1 className="t-display text-[clamp(1.5rem,4vw,2.7rem)] hidden lg:block">Configure it. We check as you go.</h1>
          {/*
            Two groups, and they are separate on purpose. The target buttons
            scroll sideways when the screen is narrow, and an ancestor with
            overflow-x also clips anything positioned out of it, which left the
            preset panel present in the DOM and invisible on screen. Anything
            that opens a panel therefore lives outside the scrolling group.
          */}
          <div className="flex flex-wrap items-center gap-2 w-full lg:w-auto">
            <div className="flex items-center gap-2 overflow-x-auto no-bar min-w-0">
              {(Object.keys(TARGET_LABEL) as Target[]).map((t) => (
                <button
                  key={t}
                  onClick={() => changeTarget(t)}
                  disabled={switching}
                  className={`btn btn-sm shrink-0 ${target === t ? "btn-primary" : "btn-ghost"}`}
            ...
            <div className="flex items-center gap-2 ml-auto lg:ml-0">
              {/* Always reachable, including half way through a configuration. */}
              <PresetMenu
                presets={presets}
                onLoad={loadPreset}
              ...
              />
              {isAdmin && <SavePreset lines={lines} target={target} blocking={errors} />}
            </div>
          </div>
        </div>
      </header>
      ...
      {presetNote && (
        <div className="panel border-l-2 border-l-warn px-4 py-2.5 mb-4 flex items-center gap-3 pop">
          <p className="text-[12.5px] text-ink-1 flex-1 min-w-0 leading-relaxed">{presetNote}</p>
          <button
          ...
          <div className={`${pane === "build" ? "block" : "hidden"} lg:block panel`}>
            ...
            <div className="p-4 border-t border-[var(--line)] space-y-2">
              ...
              <p className="t-data text-[10px] text-ink-3 leading-relaxed">
                Every configuration is priced individually. Send it and we reply within one working day with a full price, delivered and cleared.
      ...
      <div className="lg:hidden fixed bottom-0 inset-x-0 z-40 bg-[var(--color-surface)] border-t border-[var(--line)]">
        ...
        <div className="flex items-center justify-between gap-3 px-4 py-1.5 border-b border-[var(--line)]">
          <span className="t-data text-[10.5px] text-ink-2">
            {summary.power.peakW > 0 ? `${summary.power.peakW.toLocaleString("en-GB")} W · ${summary.power.amps230} A` : "no parts yet"}
          </span>
