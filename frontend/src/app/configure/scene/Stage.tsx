...
import { MM_PER_UNIT, layout, ghosts, type Box, type Interior, type Placement } from "@supercomputers/shared/build3d";
import { TARGET_LABEL, type BuildLine, type Target } from "@supercomputers/shared";
...
export default function Stage({ lines, target, dragKind, onDropPart, onDragKind, errorIds, chassisConflict }: StageProps) {
  ...
  /**
   * How many parts are actually drawn in red.
   *
   * It used to be errorIds.length, which is the number of products mentioned by
   * a blocking finding, and those two are not the same thing. A finding names
   * the part and the thing it conflicts with, so a power supply that will not
   * mount in the case counted as two parts in conflict, and the count did not
   * move when the real problem changed: reduce the supplies from two to one and
   * the blocking count fell from 2 to 1 while this badge still read "2 parts
   * conflict".
   *
   * A badge on the viewport should describe the viewport. The chassis is
   * excluded because it is not drawn as a part, it is the cage, and it turns
   * red on its own.
   */
  const flaggedCount = useMemo(() => {
    const chassisId = lines.find((l) => l.product.kind === "chassis")?.product.id;
    const flagged = new Set<string>();
    for (const p of placements) {
      if (p.id === chassisId) continue;
      if (p.clips || errorIds.includes(p.id)) flagged.add(p.id);
    }
    return flagged.size;
  }, [placements, errorIds, lines]);
  const rackLine = lines.find((l) => l.product.kind === "rack");
      ...
      <div className="absolute top-3 right-3 flex flex-col items-end gap-1.5">
        ...
        {flaggedCount > 0 && (
          <span className="pill pill-err pointer-events-none">
            {flaggedCount} part{flaggedCount > 1 ? "s" : ""} conflict
          </span>
        )}
      </div>
