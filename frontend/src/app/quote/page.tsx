...
export default async function QuotePage({
  ...
  const requestedIds = [...b.split(","), sys].filter(Boolean).map((token) => token.split("*")[0]).slice(0, 200);
  const catalog = requestedIds.length
    ? (await api<{ items: Product[] }>(`/catalog/by-ids?ids=${encodeURIComponent(requestedIds.join(","))}`)).items
    : [];
