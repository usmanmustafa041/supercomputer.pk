...
import {
  CONDITION_LABEL, KIND_LABEL, type Condition, type Kind, type Query, type Segment, type SearchResult,
} from "@supercomputers/shared";

export const metadata: Metadata = {
  title: "Catalogue",
  description: "The full catalogue, with filters for category, condition, brand and budget.",
};
...
export default async function CatalogPage({ searchParams }: { searchParams: Promise<SP> }) {
  ...
  for (const [key, value] of Object.entries({
    kind: q.kind?.join(","), condition: q.condition?.join(","), segment: q.segment?.join(","),
    brand: q.brand?.join(","), tags: q.tags?.join(","), q: q.text, stock: q.inStockOnly ? "1" : undefined,
    min: q.minPkr, max: q.maxPkr, sort: q.sort, page: q.page, perPage: q.perPage,
  })) if (value !== undefined && value !== "") params.set(key, String(value));
  const res = await api<SearchResult>(`/catalog?${params.toString()}`);
  const activeCount =
    q.kind!.length + q.condition!.length + q.segment!.length + q.brand!.length + q.tags!.length + (q.inStockOnly ? 1 : 0);

  const heading =
    q.kind?.length === 1 ? KIND_LABEL[q.kind[0]] : q.text ? `Results for “${q.text}”` : "The full catalogue";

  return (
    <div className="shell py-9 md:py-12">
      <header className="mb-8">
        <p className="t-eyebrow mb-2.5">
          {res.total.toLocaleString("en-GB")} parts
        </p>
