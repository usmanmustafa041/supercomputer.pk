...
export function boardBox(mb: Motherboard, it: Interior): Box {
  ...
  if (it.rack) {
    ...
    return {
      ...
      pos: { x: it.width - s.h / 2 - 20, y: 8, z: it.depth - s.w / 2 - 30 },
    };
  }
  ...
  for (let i = 0; i < qty; i++) {
    ...
    if (it.rack) {
      ...
      out.push({
        ...
        pos: {
          x: CARD_HEIGHT / 2 + 24,
          y: it.height - 190 - slot * PCIE_PITCH - thickness / 2,
          z: it.trayZ + len / 2 + 10,
        },
      });
    }
...
export function storageBoxes(drive: Storage, qty: number, it: Interior, start = 0): Box[] {
  ...
  for (let i = 0; i < Math.min(qty, 12); i++) {
    const n = start + i;
    if (m2) {
      out.push({ size, pos: { x: it.rack ? it.width - 60 : 60, y: it.rack ? 12 : it.height - 300 - n * 16, z: 140 + (it.rack ? n * 30 : 0) } });
    } else {
      ...
      out.push({
        size,
        pos: it.rack
          ? { x: 30 + col * (size.x + 6), y: size.y / 2 + 8 + row * (size.y + 6), z: size.z / 2 + 15 }
...
export function layout(
  ...
  const add = (id: string, kind: Kind, label: string, box: Box) =>
    placements.push({ id, kind, label, box, clips: !fitsInside(box, it) });
  ...
  for (const { product, qty } of get("storage")) {
    for (const b of storageBoxes(product, qty, it, driveIndex)) add(product.id, "storage", product.model, b);
    driveIndex += qty;
  }
